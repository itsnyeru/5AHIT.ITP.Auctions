...
public class AuthenticationService : AuthenticationStateProvider, IAuthService {
    ...
    public async override Task<AuthenticationState> GetAuthenticationStateAsync() {
        try {
            ...
            int id = Convert.ToInt32(Get("Identity"));
    ...
    public async Task<bool> Login(object value) {
    ...
    public async Task<bool> Login(string identifier, string password) {
        ...
        var token = GenerateToken(new Dictionary<string, object> { { "Identity", user.Id } });
    ...
    private string GenerateToken(Dictionary<string, object> claims) {
        ...
        var token = new JwtSecurityToken(
            "localhost",
            "localhost",
            claims.Select(c => new Claim(c.Key, Convert.ToString(c.Value))).ToArray(),
